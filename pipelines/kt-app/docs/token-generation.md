...
2. **Transparency Pass** (`script/tools/add_token_transparency_bg_sample.py`)
  - Learns the background greys from `config/defaults/tts-token/input/token-bg-sample.png`
...
### 2.5. Custom Tokens (Double-Stacked Tokens)

Some teams have double-stacked tokens on their marker guides where two separate tokens are printed overlapping (e.g., "Values 1 & 2"). The extraction process automatically skips these and expects manually cropped versions.

**Teams with double-stacked tokens:**
- **brood-brothers**: Crossfire tokens (diagonal offset)
- **hearthkyn-salvagers**: Grudge tokens (vertical stack)
- **nemesis-claw**: Prescience points (horizontal side-by-side)
- **novitiates**: Faith points (horizontal side-by-side)
- **wrecka-krew**: Wrecka points (similar to Crossfire)

**How to add custom tokens:**

1. The extraction will skip double-stacked tokens with a warning:
   ```
   ⚠ Skipping double-stacked token (add to custom-tokens/): Grudge tokens Values 1
   ```

2. Manually crop the token from the marker guide image and save it to:
   ```
   config/teams/{team-name}/custom-tokens/{token-name}.png
   ```
   
   **Naming convention:**
   - Use lowercase with hyphens
   - Optionally prefix with team name: `{team-name}-{token-name}.png` (will be auto-stripped)
   - Examples:
     - `grudge.png` or `hearthkyn-salvagers-grudge.png`
     - `crossfire.png` or `brood-brothers-crossfire.png`

3. The extraction process will automatically:
   - Copy custom tokens to the extraction output
   - Apply transparency processing
   - Include them in the packaged tokens

**Example workflow:**
```bash
# 1. Extract tokens (skips double-stacked)
poetry run python script/tools/extract_tokens.py --team hearthkyn-salvagers

# 2. Manually crop and save to config/teams/hearthkyn-salvagers/custom-tokens/grudge.png

# 3. Re-run extraction (picks up custom token)
poetry run python script/tools/extract_tokens.py --team hearthkyn-salvagers

# 4. Apply transparency to all tokens (including custom)
poetry run python script/tools/add_token_transparency_bg_sample.py --team hearthkyn-salvagers \
  --bg-sample config/defaults/tts-token/input/token-bg-sample.png

# 5. Package tokens
poetry run python script/run_pipeline.py --team hearthkyn-salvagers --step tokens
```

**Notes:**
- Custom tokens should be cropped to show only the top/main token
- Use the same white background as the marker guide (transparency will be applied automatically)
- The shape (operative/round) is inferred from the filename (e.g., "defence" → round)
...
poetry run python script/tools/add_token_transparency_bg_sample.py --team farstalker-kinband \
  --bg-sample config/defaults/tts-token/input/token-bg-sample.png --threshold 14

# More aggressive (removes more background, may lose some detail)
poetry run python script/tools/add_token_transparency_bg_sample.py --team farstalker-kinband \
  --bg-sample config/defaults/tts-token/input/token-bg-sample.png --threshold 22
```
