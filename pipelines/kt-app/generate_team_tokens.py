...
def extract_tokens(team_slug):
    ...
    processed_pdf = Path(f'layers/kt-app/processed/{team_slug}/{team_slug}-faction-rules.pdf')
...
def process_tokens(team_slug):
    ...
    # Check if tokens exist in layers/kt-app/processed/{team}/token/
    extracted_dir = Path(f'layers/kt-app/processed/{team_slug}/token')
    ...
    cmd = [
        ...
        '--team', team_slug,
        '--tokens-dir', 'layers/kt-app/processed'
    ]
    result = subprocess.run(cmd, capture_output=False)
