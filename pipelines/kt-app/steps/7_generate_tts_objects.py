...
def get_default_mesh_url() -> str:
    ...
    # Assets generated by step 6 in output_v3/{team}/tts/
    return ""
...
class TTSObjectGenerator:
    ...
    def process_team(self, team: str) -> bool:
        ...
        # Get mesh and texture URLs from step 6 output
        mesh_url = self._build_tts_asset_url(team, f"{team}-card-box.obj")
        texture_url = self._build_tts_asset_url(team, f"{team}-card-box-texture.jpg")
    ...
    def _build_tts_asset_url(self, team: str, filename: str) -> str:
        """Build GitHub raw URL for TTS asset (mesh/texture)."""
        encoded_filename = filename.replace(' ', '%20')
        return f"https://raw.githubusercontent.com/Wen-Qualtu/kt-datacards/refactor-kt-app-pipeline/output_v3/{team}/tts/{encoded_filename}"
    ...
    def _create_token_bag(self, team: str, team_config: Dict) -> Optional[Dict[str, Any]]:
        """Create a token bag with infinite dispensers for teams with tokens_ready."""
        ...
        # Get dispenser mesh URL (square bag mesh)
        dispenser_mesh_url = "https://raw.githubusercontent.com/Wen-Qualtu/kt-datacards/refactor-kt-app-pipeline/config/defaults/tts-token/square-bag-mesh.obj"
        
        # Build token dispensers
        dispensers = []
        for idx, token in enumerate(tokens):
            token_name = token.get('name', 'Unknown Token')
            token_type = token.get('type', 'token')
            token_shape = token.get('shape', 'token')
            ...
            # Determine scale based on token shape
            scale = 1.0
            if token_shape == 'operative':
                scale = 1.15
            elif token_shape == 'round':
                scale = 0.8
            ...
            # Create locked child copy (displays on top of dispenser)
            locked_child = json.loads(json.dumps(token_obj))
            ...
            # Create infinite bag dispenser
            dispenser = {
                "GUID": generate_guid(f"{team}:dispenser:{token_name}"),
        ...
        # Get token bag mesh and icon URLs
        token_bag_mesh_url = self._build_tts_asset_url(team, f"{team}-token-bag.obj")
        # Icon URL - use first team icon or fallback
        icon_url = f"https://raw.githubusercontent.com/Wen-Qualtu/kt-datacards/refactor-kt-app-pipeline/output_v3/{team}/tts/{team}-icon.png"
        
        # Create icon tile child object
        icon_tile = {
            "GUID": generate_guid(f"{team}:token-bag:icon"),
            "Name": "Custom_Tile",
            "Transform": {
                "posX": 0.0,
                "posY": -0.5,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Step 7: Generate TTS Objects'
    )
    parser.add_argument(
        '--classified-dir',
        type=Path,
