"""
Step 7: Generate TTS Objects from Extracted Cards
...
Input:  output_v3/{team}/cards/**/*.png
Output: tts_objects_v3/{team}/*.json
        tts_objects_v3/.tts-metadata.json (full tracking)
...
def prepare_clean_tokens(team_name: str, workspace_root: Path) -> Tuple[Optional[Path], Dict[str, str]]:
    """Generate cleaned/transparent tokens in output_v3/{team}/tokens from extracted metadata.
    
    If team has tokens_ready=true in config, uses existing output_v3/{team}/tokens/ as-is
    without re-extracting from layers/kt-app/extracted/.
    """
    ...
    if team_data.get("tokens_ready", False):
        ...
        final_tokens_dir = workspace_root / "output_v3" / team_name / "tokens"
    ...
    extracted_team_dir = workspace_root / "layers" / "kt-app" / "extracted" / team_name
    ...
    if not extracted_tokens_dir.exists() or not metadata_path.exists():
        # Fallback: check if tokens already exist in output_v3
        output_tokens_dir = workspace_root / "output_v3" / team_name / "tokens"
        if output_tokens_dir.exists() and list(output_tokens_dir.glob("*.png")):
            output_name_map: Dict[str, str] = {}
            for token_path in sorted(output_tokens_dir.glob("*.png")):
                display_name = token_path.stem.replace("-", " ").replace("_", " ").title()
                output_name_map[token_path.name] = display_name
            logger.info("Using existing tokens for %s (%d tokens)", team_name, len(output_name_map))
            return output_tokens_dir, output_name_map
        return None, {}
    ...
    # Apply transparency + template-fit directly in step 7
...
def find_tokens_dir(team_name: str, workspace_root: Path) -> Optional[Path]:
    ...
    output_tokens = workspace_root / "output_v3" / team_name / "tokens"
    if output_tokens.exists():
        return output_tokens

    extracted_tokens = workspace_root / "layers" / "kt-app" / "extracted" / team_name / "tokens"
...
def build_raw_url(file_path: Path, workspace_root: Path, branch: str = "refactor-kt-app-pipeline") -> str:
...
def register_image_and_get_url(
    ...
    workspace_root: Path,
    registry: ComponentRegistry,
    branch: str = "refactor-kt-app-pipeline"
) -> str:
    """
    ...
    for type_dir in cards_dir.iterdir():
        ...
        if type_dir.name == 'ploys':
            for ploy_subdir in type_dir.iterdir():
                ...
                for img_path in ploy_subdir.glob('*.png'):
                    name = img_path.stem
...
def generate_team_tts(team_dir: Path, output_dir: Path, registry: ComponentRegistry, branch: str = "refactor-kt-app-pipeline") -> bool:
    """
    ...
    # Copy final TTS save file to tts_objects_v3 for easy access
    tts_objects_dir = workspace_root / "tts_objects_v3" / team_name
    tts_objects_dir.mkdir(parents=True, exist_ok=True)
    final_save_name = f"{team_display_name} Cards.json"
    final_save_path = tts_objects_dir / final_save_name
    shutil.copy2(cardbox_file, final_save_path)
    logger.info("Copied TTS save to %s", final_save_path)
...
def main():
    ...
    parser.add_argument('--branch', default='refactor-kt-app-pipeline',
                       help='Git branch for GitHub raw URLs (default: refactor-kt-app-pipeline)')
    ...
    output_dir = workspace_dir / 'output_v3'
    tts_output_dir = workspace_dir / 'tts_objects_v3'
    metadata_file = tts_output_dir / '.tts-metadata.json'
