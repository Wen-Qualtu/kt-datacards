...
class TeamTokenBagGenerator:
    ...
    def generate_team_bag(self, team_name: str, token_bags: List[Dict]) -> Dict:
        ...
        with open(json_file) as f:
            ...
            if 'ObjectStates' in data and len(data['ObjectStates']) > 0:
                ...
                if 'ContainedObjects' in bag_obj and len(bag_obj['ContainedObjects']) > 0:
                    ...
                    # Deterministic GUID per token (stable across reruns)
                    nickname = token.get('Nickname', json_file.stem)
                    token['GUID'] = generator.generate_guid(f"{args.team}:{nickname}:token_in_team_bag")
